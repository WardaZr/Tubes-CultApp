...
public class UserNoteHelper {
    ...
    private static UserNoteHelper INSTANCE;

    private static SQLiteDatabase database;

    private UserNoteHelper(Context context) {
        dataBaseHelper = new DatabaseHelper(context);
    }

    public static UserNoteHelper getInstance(Context context) {
        if (INSTANCE == null) {
            synchronized (SQLiteOpenHelper.class) {
                if (INSTANCE == null) {
                    INSTANCE = new UserNoteHelper(context);
                }
            }
        }
        return INSTANCE;
