...
public class ContentNoteHelper {
    private static final String DATABASE_TABLE = TABLE_NAME;
    private static DatabaseHelper dataBaseHelper;
    private static ContentNoteHelper INSTANCE;

    private static SQLiteDatabase database;

    private ContentNoteHelper(Context context) {
        dataBaseHelper = new DatabaseHelper(context);
    }

    public static ContentNoteHelper getInstance(Context context) {
        if (INSTANCE == null) {
            synchronized (SQLiteOpenHelper.class) {
                if (INSTANCE == null) {
                    INSTANCE = new ContentNoteHelper(context);
                }
            }
        }
        return INSTANCE;
